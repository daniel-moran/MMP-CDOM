...
update: 23/09/2026 This script includes an extra chunck of code to reset the baseline for all the scans in this batch - the actual baseline was taken before the instrument had stabilised so the actual blanks and scans had drifted from the zero baseline - fortunalty the actual blanks and samples were scanned after the instrument had stabilised and the output data could be ~corrected to a zero baseline - using the initial blank as the baseline offset. Also see comments and records in the batch folder for further details
...
copy file path here:
R:\Lagoon_WQ\results\CDOM\2025-26 report\CYA\CDOM-20260121_CYA074-078_&_JCE520-521
---
...
Batch_info <- read.csv(file_list[1]) 
...
driftplot <- ggplot(CDOM_blanks, aes(x = Wavelength_nm, y = Absorbance, group = Analysis_time, colour = Sample)) + 
  ...
  ylab('Absorbance') +
  xlab('Wavelength_nm') +
  scale_y_continuous(minor_breaks = NULL, breaks=seq(-0.02,0.02,0.01),limits=c(-0.2,0.2)) +
...
plotfilename <- paste(batch,"_driftplot_initial", ".jpg",sep = "")
ggsave(plotfilename, path=plotpath)
}
```

### the baseline (on the spec) for this batch of scans was applied too early while the instrument was still warming up. however the start and end blanks were still saved properly and were scanned after the instrument had stabilised so they reflect the working baseline that the batch was scanned against, so we can use the existing "start" blank to correct all the raw absorbace values to a meaningful baseline

##the following lines are applying this correction to reset the "baseline" - for this chunk of code the working baseline is the 1st blank scan "milliq_01_start"

```{r}
library(tidyr)
library(data.table)
# create a copy of CDOM_data
  CDOM_baseline <- CDOM_data
  is.data.table(CDOM_baseline)
  CDOM_baseline <- data.table(CDOM_baseline)
  is.data.table(CDOM_baseline)

# make the initial blank the 'baseline value" for all scans
  names(CDOM_baseline)
  baseline <- CDOM_baseline[Sample == "milliq_01_start", Absorbance, by=Wavelength_nm] # make milliq_01_start the baseline
  setnames(baseline, "Absorbance", "Abs_baseline") #change the column name in baseline from "Absorbance" to "Abs_baseline"
  CDOM_baseline <- CDOM_baseline[baseline, on="Wavelength_nm", allow.cartesian=TRUE]
  baseline <- NULL
  rm(baseline)
  names(CDOM_baseline)
  
  print(CDOM_baseline[, Abs_corr := Absorbance-Abs_baseline]) # corrected raw absorbance values calculated based on the absorbance minus the actual baseline 
#  CDOM_baseline2 <- CDOM_baseline[Wavelength_nm >=250] # just data between 250-750nm
#  CDOM_baseline <- CDOM_baseline[order(Analysis_time)]
  CDOM_baseline <- CDOM_baseline[order(Analysis_time)]
  CDOM_baseline <- as.data.table(CDOM_baseline)  
  CDOM_baseline$Absorbance <- CDOM_baseline$Abs_corr #make the baseline corrected data into the Absorbance data
  CDOM_baseline$Abs_baseline <- NULL #remove the Abs_baseline data
  CDOM_baseline$Abs_corr <- NULL #remove the Abs_corr data
  CDOM_data <- CDOM_baseline #setup the working dataframe with the re-baselined data
  CDOM_blanks <- filter(CDOM_data, Sample %in% blanks_list) #setup the blank dataframe with the re-baselined data
  CDOM_baseline <- NULL
  rm(CDOM_baseline)
  
# the working dataframes are now setup with the correct baseline and the script can now continue as per the normal process

## plot the data in CDOM_blanks again, organise by samples, to look for drift
