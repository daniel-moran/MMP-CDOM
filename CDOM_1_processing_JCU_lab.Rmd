...
copy file path here:
R:\Lagoon_WQ\results\CDOM\2024-25 report\JCF\CDOM-JCU_J25-0095 #location on RWQPP
C:\R Working\MMP\CDOM\remote\CDOM\2024-25 report\JCF\CDOM-JCU_J25-0095 #remote copy for working with the script on a local drive if operating remotely
C:/R Working/MMP/CDOM/remote/CDOM/2024-25 report/JCF/CDOM-JCU_J25-0095
---
...
```{r, include=FALSE}
rm(list=ls())
setwd("C:/R Working/MMP/CDOM/remote/CDOM/2024-25 report/JCF/CDOM-JCU_J25-0095")
wd <- getwd()
pwd<- "C:/R Working/MMP/CDOM/remote/CDOM/2024-25 report/JCF"
```
...
CDOM_comp <- read.csv(file_list[1], header =  TRUE)
names(CDOM_comp)
...
metadata <- rename(metadata, short_name = NANA)
metadata <- rename(metadata, station_name_depth = Bottle.ID.for.Sample.Bottle)
metadata$station_name_depth
str_sub(metadata$station_name_depth, 7, 6) <- "_" 
metadata$station_name_depth
names(metadata)
metadata <- metadata %>% mutate(Sample = paste(metadata$Reference, ".", metadata$SampleNo., sep = ""))
names(metadata)
metadata$Sample
metadata <- metadata %>% relocate(Sample, .after = SampleNo.)
names(metadata)
metadata <- metadata %>% select(c(Event..:short_name))
names(metadata)
metadata$Sample
metadata$Sample <- str_replace(metadata$Sample, c("-"), c("."))
metadata$Sample
