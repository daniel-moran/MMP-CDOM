...
update: 10/05/2023 This script includes an extra chunck of code to reset the baseline for all the scans in this batch - the actual baseline was taken before the instrument had stabilised so the actual blanks and scans had drifted from the zero baseline - fortunalty the actual blanks and samples were scanned after the instrument had stabilised and the output data could be ~corrected to a zero baseline - using the initial blank as the baseline offset. Also see comments and records in the batch folder for further details
update: 17/06/2025 moved this script to github repo MMP-CDOM/baseline_corrections and renamed to remove now redundant version control (no longer required with move to github)
...
copy file path here:
R:\Lagoon_WQ\results\CDOM\2022-23 report\CDOM-20230116_WQQ667-671
---
...
```{r, include=FALSE}
rm(list=ls())
setwd("R:/Lagoon_WQ/results/CDOM/2022-23 report/CDOM-20230116_WQQ667-671")
wd <- getwd()
pwd<- "R:/Lagoon_WQ/results/CDOM/2022-23 report"
```
...
driftplot <- ggplot(CDOM_blanks, aes(x = Wavelength_nm, y = Absorbance, group = Analysis_time, colour = Sample)) + 
  ...
  ylab('Absorbance') +
  xlab('Wavelength_nm') +
  scale_y_continuous(minor_breaks = NULL, breaks=seq(-0.02,0.02,0.01),limits=c(-0.20,0.20)) +
...
# merge dat_order and CDOM_corr to a single data table
  ...
  CDOM_corr <- NULL
  rm(CDOM_corr)
# calculate raw absorbance values with correction for drift
  names(correctiondat)
