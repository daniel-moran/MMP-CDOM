---
title: "CDOM extract and analysis_2026"
...
#set the wd
#setwd("R:/Lagoon_WQ/results/CDOM/2025-26 report/WQR") # set your working directory # In R Markdown, each code chunk is independent regarding its working directory. If you change the directory inside a chunk, it only lasts for that specific block of code. When the chunk finishes, R resets the directory back to where the .Rmd file is saved

knitr::opts_knit$set(root.dir = 'R:/Lagoon_WQ/results/CDOM/2025-26 report/WQR') #To change the working directory for the entire notebook permanently, set the root.dir in your initial "setup" chunk
...
```{r}
#| label: read in the required data
#| eval: true

# create list of all files in the wd with drift corrected Acdom data fields
file_list_Acdom <- list.files(pattern="Acdom_drift_corr.csv", full.names=FALSE, recursive = TRUE) # recursive = TRUE allows list.files function to look in all subdirectories. if there are lots of subdirectories this can take some time
...
target_stations <- all_stations[grepl("Wav|WQR405_D0|WQR406_D0", all_stations)] #refine a list of scans to be extracted
Acdom_subset <- Acdom_all |> select(all_of(target_stations)) |> as_tibble() 
names(Acdom_subset)
Acdom_subset <- dplyr::rename(Acdom_subset, WQR405_D0_1 = WQR405_D0, WQR406_D0_1 = WQR406_D0) #rename to maintain consistent nomenclature
names(Acdom_subset)
stn_order <- names(Acdom_subset)[order(names(Acdom_subset))] #create a list of the required station that is ordered alphanumerically
#in this instance the 'reps' dont include leading zeros so the order is wacko. I've ordered it manually (below) by rearranging the output from the previous line. for extracting typical MMP data this should not be required.
...
#print the data
#path <- file.path(wd)
#write.csv(Acdom_subset,file="CDOM_holding_time_ACDOM_driftcorr.csv",row.names=F) #remove # to print the data again
...
Acdom_subset_long <- Acdom_subset_long |> dplyr::rename(sample_id_x = sample_id) #the extracted dataset has reps in the sample_id field but they are not padded with leading zeros so the order is wacko
Acdom_subset_long <- Acdom_subset_long |> mutate(sample_id_rep = str_sub(sample_id_x, start = 11)) #separate the 'rep' component
Acdom_subset_long <- Acdom_subset_long |> mutate(sample_id = str_sub(sample_id_x, start = 1, end = 10)) #separate the 'sample_id' component
Acdom_subset_long$sample_id_rep <- str_pad(Acdom_subset_long$sample_id_rep, width = 2, pad = "0") #pad the rep component with a leading zero
Acdom_subset_long <- Acdom_subset_long |> mutate(sample_id = paste0(sample_id, sample_id_rep)) #append the sample_id field with the padded rep
Acdom_subset_long <- Acdom_subset_long |> mutate(station = str_sub(sample_id_x, start = 1, end = 6)) #create a field to group the samples by station
Acdom_subset_long$sample_id_x <- NULL #sample_id_x no longer needed so remove
Acdom_subset_long <- Acdom_subset_long |> relocate(sample_id, .after = Wavelength_nm) #visually tidy the dataframe by moving the fields into a logical order
#create a field to group the samples by the analytical batch (time since sampled when the rep was analysed)
...
Acdom_subset_long <- Acdom_subset_long |> group_by(analysis_time) #group by analysis_time
group_keys(Acdom_subset_long)
...
```{r}
#| label: extract Acdom from specific wavelengths, visualise and analyse
#| eval: true

#extract Acdom_443, visualise the data from this wavelngth, compare using multivariate analysis 

#subset the data
Acdom_443_long <- Acdom_subset_long |> filter(Wavelength_nm == 443.0)
#Acdom_subset_long$sample_id_rep <- NULL
Acdom_443_long <- Acdom_443_long |> group_by(station, analysis_time)
group_keys(Acdom_443_long)

#create factors to feed into the ANOVA and plots
...
#
#Independence
##twelve seperate replicates were subsampled from a single niskin bottle collected at each station. each replicate was individually filtered following standard MMP methods (ref). four separate replicates were analysed and then discarded for each batch of analysis. No samples were analysed more than 1 time. The replicate samples are independent between groups and within each group.
#
#Normality
...
hist(ANOVA_443$residuals) #its kind of skewed to the right... but its a small dataset...I think its fine but could benefit from review
#
...
#Tukey test again but with only the factor with 3 levels - analysis_time
...
#Tukey test on the interaction only - station:analysis_time
...
#migrate to Rmd - I have migrated and pushed sections of code into chuncks (7/5/26) and reviewed the chunks (19/5/26). should all work now
...
##  490 (KD490), 750 (TSS??), a lower wavelength. review the lit and talk to Caro
